(() => {
  ...
  function tryStart() {
    imagesLoaded++;
    if (imagesLoaded === 2) start(); // Nur wenn beide Bilder fertig
  }

  sheepImg.onload = tryStart;
  galerieImg.onload = tryStart;
  ...
  function start() {
    ...
    function drawIconStar(starObj, img, fov) {
    ...
    canvas.addEventListener('click', async e => {
      ...
      if (checkHit(sheepStar)) {
        ...
        if (window.openGalerieLightbox) await window.openGalerieLightbox();
      }
    });
  }
})();
